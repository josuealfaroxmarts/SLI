...
class TrafitecDescuentos(models.Model):
    ...
    def _check_monto_abono(self):
        for rec in self:
            if rec.monto and rec.abono_total:
                if rec.abono_total > rec.monto:
                    raise UserError(
                        "Aviso!\nEl abono del descuento ({}) debe ser".format(
                            rec.abono_total
                        )
                        + " manor o igual al saldo del descuento ({})".format(
                            rec.monto
                        )
                        + "."
                    )
    ...
    def unlink(self):
        ...
        return super(TrafitecDescuentos, self).unlink()

    @api.depends("abono_id.name")
    def _compute_abono_total(self):
        for rec in self:
            rec.abono_total = sum(line.name for line in rec.abono_id)

    @api.depends("abono_total", "monto")
    def _compute_saldo(self):
        for rec in self:
            if rec.abono_total:
                rec.saldo = rec.monto - rec.abono_total
            else:
                rec.saldo = rec.monto
    ...
    def _onchange_viaje(self):
        for rec in self:
            if rec.viaje_id:
                rec.asociado_id = rec.viaje_id.asociado_id
                rec.operador_id = rec.viaje_id.operador_id
        ...
        for rec in self:
            ...
            if rec.viaje_id:
                if rec.viaje_id.descuento_combustible_id.id == rec.id:
                    rec.viaje_id.with_context(
                        validar_credito_cliente=False,
                        validar_cliente_moroso=False
                    ).write({"descuento_combustible_id": False})
            rec.state = "cancelado"
