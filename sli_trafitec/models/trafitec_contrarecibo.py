...
class TrafitecContrarecibo(models.Model):
    ...
    def _factura_relacionada(self, crear, vals):
        ...
        if contrarecibo:
            for cr in contrarecibo:
                folios += cr.name + ' '
            raise UserError(
                ('La carta porte ya esta en otro contra recibo ({}).'.format(folios))
            )
            ...
            if self.cobrar_descuentos == 'Todos':
                for descuento in self.descuento_id:
                    ...
                    if descuento.abono>obj_descuento.saldo:
                        raise UserError(
                            ('Error !\nEl abono del descuento ({}) es mayor al saldo del descuento ({}).'
                                .format(descuento.abono,obj_descuento.saldo))
                        )
                ...
                for descuento in self.descuento_id:
                    ...
                    if descuento.viaje_id in self.viaje_id:
                        ...
                        if descuento.abono > obj_descuento.saldo:
                            raise UserError(
                                ('Error !\nEl abono del descuento ({}) es mayor al saldo del descuento ({}).'
                                    .format(descuento.saldo,obj_descuento.saldo))
                            )
            ...
            if self.cobrar_comisiones == 'Todos los viajes':
                for comisi in self.comision_id:
                    ...
                    if obj_comisi.saldo!=comisi.saldo:
                        raise UserError(
                            ('Error !\nEl abono de comisión ({}) es mayor al saldo de la comision ({}).'
                                .format(comisi.saldo,obj_comisi.saldo))
                        )
    ...
    company_id = fields.Many2one(
        'res.company', 
        'Company', 
        default=lambda self: self.env['res.company']._company_default_get('trafitec.contrarecibo')
    )
