...
class TrafitecViajes(models.Model):
    ...
    calificaiones = fields.One2many(
        ...
        comodel_name="trafitec.clasificaciones.generales.x.viaje",
        tracking=True
    )
    ...
    def gelocalization(self):
        for rec in self:
            ...
            for geo in self.env["trafitec.routers.shipments"].search(domain):
    ...
    def Mensaje(self, mensaje):
        return {
            ...
            "view_type": "form",
            "view_mode": "form",
            "res_model": "custom.pop.message.wizard",
            "target": "new",
    ...
    celular_asociado = fields.Char(
        string="Celular asociado", 
        required=True
    )
    ...
    celular_operador = fields.Char(
        string="Celular operador", 
        required=True
    )
    info = fields.Text(
        string="Información",
    ...
    slitrack_codigo = fields.Char(
        string="Código slitrack", 
        default=""
    )
    ...
    folio_cliente = fields.Char(
        string="Folio del cliente", 
        tracking=True
    )
    suger_pago = fields.Boolean(
        string="Sugerir pago inmediato", 
        tracking=True
    )
    ...
    tamano_contenedor_uno = fields.Selection(
        [
            ("no_especificado", "No especificado"),
            ("40_pies", "40 pies"),
            ("40_pies_hc", "40 pies HC"),
            ("20_pies", "20 pies"),
            ("20_pies_hc", "20 pies HC")
        ],
    ...
    facturar_con = fields.Selection(
        [
            ("peso_convenido", "Peso Convenido"),
            ("peso_origen", "Peso Origen"),
            ("peso_destino", "Peso Destino")
        ],
