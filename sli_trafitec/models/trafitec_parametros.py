...
class trafitec_parametros(models.Model):
	...
	cr_moneda_id = fields.Many2one(
		...
		required=True
	)  # Moneda predeterminada para nuevo contra recibos.
	...
	descuento_combustible_externo_id = fields.Many2one(
		...
		comodel_name='product.product',
		help='Proveedor externo: Producto donde se obtendra el costo del combustible para los calculos.'
	)
	descuento_combustible_interno_id = fields.Many2one(
		string='Combustible interno',
		comodel_name='product.product',
		help='Autoconsumo: Producto donde se obtendra el costo del combustible para los calculos.'
	)
