...
class TrafitecGlo(models.Model):
    ...
    def ViajesAsociadoPorMunicipios(
        self,
        asociado_id=None,
        municipio_origen_id=None,
        municipio_destino_id=None
    ):

        """
        Regresa Todos los viajes del asociado de acuerdo al
        municipio origen y destino.
        """

        if (
            not asociado_id
            or not municipio_origen_id
            or not municipio_destino_id
        ):
            return []
        lista = []
        sql = """
