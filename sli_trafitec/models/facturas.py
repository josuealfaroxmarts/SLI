		...
		for f in self:
			...
			if self._context.get('type', 'out_invoice') == 'out_invoice':
				for v in f.viajes_id:
					...
					if vobj.en_factura:
						error = True
						errores += "El viaje {} ya tiene factura cliente: {}.\r\n".format(v.name, (v.factura_cliente_id.name or v.factura_cliente_id.number000 or ''))
				...
				for vcp in f.viajescp_id:
					...
					if vobj.en_cp:
						error = True
						errores += "El viaje {} ya tiene carta porte: {}.\r\n".format(vcp.name, (vcp.factura_proveedor_id.name or vcp.factura_proveedor_id.number000 or ''))
