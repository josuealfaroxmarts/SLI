...
class TrafitecProgramacionPagosX(models.Model):
	_name = "trafitec.programacion.pagos.x"
	_description="Programacion Pagos X"

	name = fields.Char(
		string="Folio", 
		help="Folio de la programación."
	)
	nombre = fields.Char(
		string="Nombre", 
		help="Nombre de la programación."
	)
	tipo = fields.Selection(
		...
		default="proveedor", 
		help="Tipo de pago para proveedor o de cliente."
	)
	...
	facturas_aplicar_id = fields.One2many(
		string="Facturas a aplicar", 
		comodel_name="trafitec.programacion.pagos.x.facturas.aplicar", 
		inverse_name="programacionpagos_id"
	)
	buscar_persona_id = fields.Many2one(
		string="Persona", 
	...
	buscar_facturas_id = fields.One2many(
		string="Facturas a buscar", 
		comodel_name="trafitec.programacion.pagos.x.facturas.buscar", 
		inverse_name="programacionpagos_id"
	)
	...
	total = fields.Float(
		string="Total", 
		help="Total de la programación de pagos", 
		default=0
	)
	detalles = fields.Char(
		string="Detalles", 
		help="Detalles de la programación."
	)
	...
	total_abonos = fields.Monetary(
		string="Total abonos", 
		compute="compute_total_abonos", 
	...
	def action_buscar_facturas(self):
		...
		facturas_dat = facturas_obj.search(
			[
				("partner_id", "=?", self.buscar_persona_id.id), 
				("state", "=", "open"), 
				("type", "=", "in_invoice"),
				("date", ">=", self.buscar_fecha_inicial), 
				("date", "<=", self.buscar_fecha_final), 
				("name", "ilike", "%" + (self.buscar_folio or "") + "%")
			],
			order="id asc"
		)
	...
	def aplica_pagos(self):
		...
		movimientos = []
		for g in grupos:
			nuevo_movimiento = self.genera_movimiento_general(empresa_actual_id, self.moneda_id.id, self.diario_id.id, g["total"], 
				g["persona_id"], "")
			nuevo_movimiento_credito_linea = self.genera_movimiento_detalle(nuevo_movimiento.id, self.diario_id.id, False, False, 
				g["persona_id"], 0, 0, g["total"], self.diario_id.default_credit_account_id.id, False, 3)
