...
class trafitec_contrarecibo2(models.Model):
    _name = 'trafitec.contrarecibo2'
    ...
    def _factura_relacionada(self,crear,vals):
        ...
        contrarecibo=self.env['trafitec.contrarecibo2'].search(condiciones)
    ...
    def _carga_cargospendientes(self):
        ...
        cargosx=[]
        cargos=self.env['trafitec.cargospendientes2']
    ...
    def _generar_nota_cargo(self, vals, tipo, parametros_obj):
        ...
        #Si no se especifica el diario, toma el configurado.
        #if self.notascargo_diario_id:
        #    diario=self.notascargo_diario_id
        ...
        #if not diario.use_for_cfdi:
        #    error = True
        #    errores +='\nEl diario no esta configurado para cfdi.'
        ...
        #diferencia=monto-c_total
        #if diferencia != 0:
        #   subtotal=self.truncar2(c_subtotal)+diferencia
        #   iva=self.truncar2(subtotal*piva)
        #   riva=self.truncar2(subtotal*priva)
        #   total=subtotal+iva+riva
    ...
    def _get_parameter_company(self,vals):
        if vals.company_id.id != False:
            ...
            company_id = self.env['res.company']._company_default_get('trafitec.contrarecibo2')
    ...
    def action_available(self):
        ...
        #cartasportes=self.env['trafitec.contrarecibo2'].search([('invoice_id','=',self.invoice_id.id),('id','!=',self.id)])
        ...
        for viaje in self.viaje_id:
            ...
            #relacion=self.env['trafitec.viajesxcontrarecibo2']
    ...
    def action_cancel(self):
        if self.state == 'Validada':
            ...
            if self.folio_diferencia:
               if self.folio_diferencia.state=='open':
    ...
    def _compute_subtotalSM(self):
            ...
            self.subtotal_gSM = self.fletes - self.mermas_antes - self.descuento_antes - self.comisiones_antes - self.prontopago_antes
    ...
    cargosadicionales_total = fields.Float(string='+Cargos adicionales', default=0, help='Total de cargos adicionales.')
    ...
    notacargo = fields.Float(string='Nota de cargo', compute='_compute_diferencia')
    ...
    company_id = fields.Many2one('res.company', 'Company',default=lambda self: self.env['res.company']._company_default_get('trafitec.contrarecibo2'))
    ...
    descuento_id = fields.One2many(comodel_name="trafitec.con.descuentos2", inverse_name="linea_id")
    comision_id = fields.One2many(comodel_name="trafitec.con.comision2", inverse_name="line_id")
    ...
    def create(self, vals):
        ...
        if 'company_id' in vals:
            vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code(
                'Trafitec.Contrarecibo2') or _('Nuevo')
        else:
            vals['name'] = self.env['ir.sequence'].next_by_code('Trafitec.Contrarecibo2') or _('Nuevo')
...
#Mike
class trafitec_cargospendientes2(models.TransientModel):
    _name = 'trafitec.cargospendientes2'
...
class trafitec_con_descuentos2(models.Model):
    _name = 'trafitec.con.descuentos2'
...
class trafitec_con_comision2(models.Model):
    _name = 'trafitec.con.comision2'
    ...
    line_id = fields.Many2one(comodel_name="trafitec.contrarecibo2", string="Contrarecibo id", ondelete='cascade')
    viaje_id = fields.Many2one('trafitec.viajes', string='Viaje ID')
    cobrado = fields.Boolean(string='Cobrado', default=False)

#Descuentos por cobrar.
class trafitec_contrarecibo_descuentosx2(models.TransientModel):
    _name = 'trafitec.contrarecibos.descuentosx2'
...
class viajesxcontrarecibo2(models.Model):
...
class trafitec_contrarecibos_cargos2(models.Model):
    _name = 'trafitec.contrarecibos.cargos2'
    ...
    contrarecibo_id = fields.Many2one(
        ...
        comodel_name='trafitec.contrarecibo2'
    )
