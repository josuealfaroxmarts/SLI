...
class trafitec_ubicaciones(models.Model):
	...
	# MODIFICACION RECIENTE ALTA DE UBICACIONES ELIMINADOS CAMPOS "TIPO DE UBICACION" Y "TIPO DE CARGA
...
class trafitec_ubi_responsable(models.Model):
	...
	etiqueta_id = fields.Many2one('trafitec.etiquetas', string='Etiqueta', required=True)
...
class trafitec_respartner(models.Model):
	...
	# Clientes
	aseguradora = fields.Boolean(string="Es aseguradora")
...
and f.partner_id={}
	...
	def _compute_unidades_txt(self):
        ...
        rfc = fields.Many2one(comodel_name="ir.attachment",
                             relation="m2m_ir_identity_card_moral_rel",
                             column1="m2m_id",
                             column2="attachment_id",
                             string="rfc")

        estatus_cliente = fields.Selection([('draft', 'Borrador'), ('aprobado', 'Aprobado'), ('rechazado', 'Rechazado')], string='Status')
...
class trafitec_asociados(models.Model):
	...
	nolicencia = fields.Char(string='No. licencia')
    ...
    nombres_moral = fields.Char(string="Nombre(s)", required=True)
    apellidos_moral = fields.Char(string="Apellido(s)", required=True)
    correo_moral = fields.Char(string="Correo Electronico", required=True)
    telefono_moral = fields.Char(string="Teléfono de contacto", required=True)
    identificacion_representante_moral = fields.Many2one(comodel_name="ir.attachment", relation="m2m_ir_identity_card_moral_rel", column1="m2m_id", column2="attachment_id", string="Identificación del representante")
    rfc_moral = fields.Char(string="RFC del representante", required=True)

    curp_moral = fields.Many2one(comodel_name="ir.attachment", relation="m2m_ir_curp_moral_rel", column1="m2m_id", column2="attachment_id", string="Curp representante legal")

    acta_moral = fields.Many2one(comodel_name="ir.attachment", relation="m2m_ir_acta_moral_rel", column1="m2m_id", column2="attachment_id", string="Acta del representante")
    domicilio_moral = fields.Many2one(comodel_name="ir.attachment", relation="m2m_ir_domicilio_moral_rel", column1="m2m_id", column2="attachment_id", string="Comprobante Domicilio")
    ...
    identificacion_representante_fisica = fields.Many2one(comodel_name="ir.attachment", relation="m2m_ir_identificacion_moral_rel", column1="m2m_id", column2="attachment_id", string="Identificación del representante")
...
class trafitec_parametros(models.Model):
	...
	cr_lineanegocio_id = fields.Many2one(comodel_name='trafitec.lineanegocio', string='Línea negocio predeterminada',
										 required=True)  # Linea de negocio para nuevo contra recibos.
