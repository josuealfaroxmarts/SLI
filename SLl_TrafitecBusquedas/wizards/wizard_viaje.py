...
class wizard_viaje(models.TransientModel):
    ...
    viajes_id = fields.Many2many(
        'trafitec.viajes', 
        string='Viajes:', 
        default=_get_viaje_seleccionados
    )
    ...
    comisiones_id = fields.Many2many(
        'trafitec.cargos', 
        string='Comisiones:', 
        required=True
    )
    ...
    def _get_viaje_sin_cr(self):
        r = self.env['trafitec.viajes'].search(['&', ('active', '=', True), 
            ('en_contrarecibo', '=', False), ('tipo_viaje', '=', 'Normal')])
        self._total()
    ...
    def _carga_viajes(self):
        self.viajes_id = []
        self.viajes_id = self.env['trafitec.viajes'].search(['&', ('asociado_id', '=', self.asociado_id.id), 
            ('active', '=', True)])
