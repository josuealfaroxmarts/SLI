...
class TrafitecReportesGenerales(models.TransientModel):
	...
	tipo = fields.Selection(
		...
		selection=
		[
			('cliente_dias_cartera', 'Dias de cartera de cliente'),
			...
			('operaciones_pedidos_estado','Estado de pedidos')
		],
		required=True
	)
...
where
f.state='open' and f.type='out_invoice'
and f.company_id={}
group by p.name
...
p.name persona,
max(p.id) persona_id,
max(case p.asociado when True then 'SI' else 'NO' end) es_asociado,
--f.date_invoice fecha,
--f.date_due fecha_vencimiento,
--current_date-f.date_due dias,
sum(f.amount_total) total,
sum(f.residual) saldo,
 ...
 sum(case when (current_date-f.date_due)>=16 and (current_date-f.date_due)<=30 then f.residual else 0 end) v_d16a30,
 sum(case when (current_date-f.date_due)>=31 and (current_date-f.date_due)<=45 then f.residual else 0 end) v_d31a45,
 sum(case when (current_date-f.date_due)>=46 and (current_date-f.date_due)<=60 then f.residual else 0 end) v_d46a60,
 
 sum(case when (current_date-f.date_due)>=61 and (current_date-f.date_due)<=75 then f.residual else 0 end) v_d61a75,
 sum(case when (current_date-f.date_due)>=76 and (current_date-f.date_due)<=90 then f.residual else 0 end) v_d76a90,
 sum(case when (current_date-f.date_due)>=91 and (current_date-f.date_due)<=105 then f.residual else 0 end) v_d91a105,
 sum(case when (current_date-f.date_due)>=106 and (current_date-f.date_due)<=120 then f.residual else 0 end) v_d106a120,
 sum(case when (current_date-f.date_due)>=121 then f.residual else 0 end) v_d121an

from account_invoice as f
  inner join res_partner p on(f.partner_id=p.id)
  left join trafitec_contrarecibo as cr on(cr.move_id=f.id)
where
f.state='open' and f.type='in_invoice'
and f.company_id={}
group by p.name
""".format(self.empresa_id())
		self.env.cr.execute(sql)
		...
		#General.
		worksheet.write_datetime('B1', datetime.datetime.now(), ff)
		worksheet.merge_range('B2:N2', self.empresa_nombre(), ft)
		
		#Cabecera.
		worksheet.write(rowi+row, coli+0, "PROVEEDOR", fc)
		worksheet.write(rowi+row, coli+1, "ES ASOCIADO", fc)
		worksheet.write(rowi+row, coli+2, "TOTAL", fc)
		worksheet.write(rowi+row, coli+3, "SALDO", fc)
		worksheet.write(rowi+row, coli+4, "D 1-15", fc)
		worksheet.write(rowi+row, coli+5, "D 16-30", fc)
		worksheet.write(rowi+row, coli+6, "D 31-45", fc)
		worksheet.write(rowi+row, coli+7, "D 46-60", fc)
		
		worksheet.write(rowi+row, coli+8, "D 61-75", fc)
		worksheet.write(rowi+row, coli+9, "D 76-90", fc)
		worksheet.write(rowi+row, coli+10, "D 91-105", fc)
		worksheet.write(rowi+row, coli+11, "D 106-120", fc)
		worksheet.write(rowi+row, coli+12, "D 121-N", fc)
		
		#Datos.
		for f in facturas:
			row += 1
			worksheet.write(rowi+row, coli+0, f['persona'])
			worksheet.write(rowi+row, coli+1, f['es_asociado'])
			worksheet.write_number(rowi+row, coli+2, f['total'], fm)
			worksheet.write_number(rowi+row, coli+3, f['saldo'], fm)
			worksheet.write_number(rowi+row, coli+4, f['v_d1a15'], fm)
			worksheet.write_number(rowi+row, coli+5, f['v_d16a30'], fm)
			worksheet.write_number(rowi+row, coli+6, f['v_d31a45'], fm)
			worksheet.write_number(rowi+row, coli+7, f['v_d46a60'], fm)
			
			worksheet.write_number(rowi+row, coli+8, f['v_d61a75'], fm)
			worksheet.write_number(rowi+row, coli+9, f['v_d76a90'], fm)
			worksheet.write_number(rowi+row, coli+10, f['v_d91a105'], fm)
			worksheet.write_number(rowi+row, coli+11, f['v_d106a120'], fm)
			worksheet.write_number(rowi+row, coli+12, f['v_d121an'], fm)
